...
> ⚖️ **Список ниже написан в предпосылке «одна нода — один владелец».** Если нода становится
> общей (несколько арендаторов), половина пунктов меняет класс с «терпимо» на «блокер», и
> добавляется то, чего здесь нет вовсе: квоты диска на том, честная очередь сборок,
> per-tenant docker-сеть, лимит сертификатов на зону.
...
- **SQLite + многопоточность:** оркестратор (`asyncio.to_thread`) и web-воркеры
  ...
  🟡 *Обе БД получили `PRAGMA journal_mode=WAL` + `busy_timeout` + параметры пула 27.07.2026. 🔴 Прежняя формулировка этого абзаца была
  НЕВЕРНА по механизму: незакоммиченный писатель держит RESERVED, а не EXCLUSIVE, и читателей
  сам по себе не блокирует (проверено на `journal_mode=delete`). WAL спасает читателя при
  спилле страничного кэша долгой транзакции и в окне коммита, но **не даёт двух писателей**:
  вторая параллельная запись получает `database is locked` и с WAL. Гонки записи по-прежнему
  решаются только сериализацией писателей или переходом на PostgreSQL. Деплоерскую БД уже
  чинили после боевого инцидента (ADR-137), облачную — нет. Ожидаемый отказ:
  `database is locked` на всю платформу. Полдня работы, полезно уже сегодня.*
...
- **Жёстко зашитый Dockerfile приложений** (только python:3.9 + uvicorn `main:app`) —
  ...
  🔴 *Второе следствие того же места, важное для плотности: `generate_dockerfile` делает
  `COPY . .` **до** `RUN pip install` (`docker_manager.py:31-36`) → слой зависимостей
  уникален для каждого арендатора и пересобирается при любой правке. На общей ноде именно
  диск, а не RAM, оказывается реальным потолком числа арендаторов.*
