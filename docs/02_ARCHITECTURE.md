...
> per-tenant docker-сеть, WAL для `cloud.db`, двенадцать процессных локов контрол-плейна,
> лимит сертификатов на зону. Полная ревизия — [`28_HOSTING_BRIDGE.md`](28_HOSTING_BRIDGE.md)
> §5.5 (изоляция: что есть, чего нет) и §4.2 (гейты T1–T19).
> Зачем это вообще понадобилось — [`27_PRICING_ECONOMICS.md`](27_PRICING_ECONOMICS.md) §7 п.23:
> разница между плотностью 1 и 25 арендаторов на ноде равна **5 646 ₽ на одном клиенте**.
...
- **SQLite + многопоточность:** оркестратор (`asyncio.to_thread`) и web-воркеры
  ...
  🟡 *Обе БД получили `PRAGMA journal_mode=WAL` + `busy_timeout` + параметры пула 27.07.2026
  (`app/cloud/database.py`, `app/database.py`). 🔴 Прежняя формулировка этого абзаца была
...
- **Жёстко зашитый Dockerfile приложений** (только python:3.9 + uvicorn `main:app`) —
  ...
  диск, а не RAM, оказывается реальным потолком числа арендаторов (`28` §5.1).*
